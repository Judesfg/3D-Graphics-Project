...
public class Model {
  ...
  public void render(GL3 gl, Mat4 modelMatrix) {
    ...
    shader.setFloat(gl, "light.cutoff", (float)Math.cos(Math.toRadians(light.spotlightAngle)));
 /* 
    Vec3 aPos = new Vec3(-1f,0f,1f);
    Vec3 lightDir = light.getPosition();
    lightDir.subtract(aPos);
    lightDir.normalize();
    Vec3 nDirection = light.getDirection();
    nDirection.x = -nDirection.x;
    nDirection.y = -nDirection.y;
    nDirection.z = -nDirection.z;
    nDirection.normalize();
    float theta = lightDir.dotProduct(nDirection);

    System.out.println((float)Math.cos(Math.toRadians(light.spotlightAngle)));
